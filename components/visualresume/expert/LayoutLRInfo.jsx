...
const LayoutLRInfo = (props) => {
  ...
  const [sequence, setSeuence] = useState(visualresumeexp.layout.sequence)
  
  
  
  const [sequencelr, setSequencelr] = useState(visualresumeexp.layout.sequencelr)
  
  const [list, setList] = useState(visualresumeexp.layout.list);
  const [listLR, setListLR] = useState(visualresumeexp.layout.listLR);
    ...
    return (
      sequencelr.right.map((l, i)=> {
        let str = l + "Class"
          ...
          if(l == "competenciesInfo" || l == "workexpInfo"){
          return (
              <div>
                <button className = {classes[str]} >{name[l]}</button>
                <button className = "btn btn-sm" onClick = {() => editClick(l)}><i class="fas fa-edit"></i></button>
