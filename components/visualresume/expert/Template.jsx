...
const Template = (props) => {
 ...
 const vr = (data, layoutInfoDisplay, userInfoDisplay) => {
   setValues({...values, visualresumeexp: data, list: props.componentSequence(data, name, email, photo), 
							layoutInfoDisplay: layoutInfoDisplay, userInfoDisplay: userInfoDisplay});
 }
  const personalInformation = (data) =>{
    console.log(props.componentSequence(data.visualresumeexp, data.name, data.email, data.photo))
    setValues({...values, name: data.name, username: data.username, email: data.email, photo: data.photo, visualresumeexp:data.visualresumeexp, 
               list: props.componentSequence(data.visualresumeexp, data.name, data.email, data.photo)
               ,showLeftBlock: true})
  }
              ...
              <Row>
                ...
                <Col xs = "2" lg = "1">
                  <div style = {{width: `50px`, height: `50px`, backgroundColor: `${font}`, marginTop: `10px`}}></div>
								...
								<Col xs= "12" lg = "4">
                  <UserInformationexpert vr = {vr} pr = {personalInformation} type={props.type} template = {props.template} userInfoDisplay = {userInfoDisplay} ref = {editSection} />
                </Col>
