...
const ToolSkillsInfo = (props) =>{
  ...
  const c = "toolSkillsInfoDisplay";
  
  const [list, setList] = useState(visualresumepro.toolSkillsInformation.value);
  ...
  const handleDelete = (idx) => {
    ...
    arr.splice(idx, 1);
    setList(arr);
    visualresumeCopy.toolSkillsInformation.value = arr;
    vr(visualresumeCopy)
  }
  ...
  const handleChange = (n, idx) => e => {
      let visualresumeCopy = visualresumepro;
      visualresumeCopy.toolSkillsInformation.value[idx][n] = e.target.value;
      vr(visualresumeCopy);
    };
  
  const handleTitle = e => {
    let visualresumeCopy = visualresumepro;
    visualresumeCopy.toolSkillsInformation.title = e.target.value;
    vr(visualresumeCopy);
  }
  
  return(
  <div className= "container">
      <label className="lead">Tools Skills</label>
      <input 
        type = "text"
        className="form-control"
        defaultValue={visualresumepro.toolSkillsInformation.title}
