...
const RightBlockWorkExp = (props) =>{
  ...
  return (
    <div id = {id} style = {{height: `${height}`, width: `${width}`, position: `absolute`, top: `${top}`, left: `${left}`, color: `${bg}`, fontFamily: `calibri`, fontSize: `${size}`, textAlign: `left`}}>
      ...
      <p style = {{top: `${top1}`, position: `absolute`}}><span style = {{fontWeight: `bold`, color: `${font}`}}>{org}</span>  <span style = {{fontStyle: `italic`}}>{startD}-{endD}</span></p>
      <p style = {{top: `${top2}`, position: `absolute`, fontWeight: `bold`}}>{desg}</p>
      {role && <p style = {{top: `${top3}`, position: `absolute`, color: `${font}`,fontStyle: `italic`}}>Roles & Responsibilities</p>}
    </div>
  )
}
