...
const PorInfo = (props) =>{
  ...
  const c = "porInfoDisplay";
  
  const [list, setList] = useState(visualresumepro.porInformation.value);
  ...
  const handleAdd = () => {
    ...
    setList(arr);
    visualresumeCopy.porInformation.value = arr;
    vr(visualresumeCopy);
  }
    ...
    list.map((l, i)=>{
      return (
        <div className = "form-group mt-2" key = {i}>
          <label className="lead">POR {i+1} Title</label>
          <input 
            type = "text"
          ...
          <label className="lead">POR {i+1} Description</label>
          <ReactQuill
        ...
        />
          ...
          <label className="lead">Project {i+1} Event/Committee/Club</label>
          <input  type="checkbox" 
            className = "mt-2 ml-2"
            style= {{width:`20px`, height:`20px`, lineHeight: `20px`}}
            checked = {l.event.optional}
            onChange = {handleChangeCheck("event", i)} ></input>
          {l.event.optional && <input 
            type = "text"
            className="form-control"
            onChange= {handleChangeOptional("event", "value" ,i)}
            defaultValue = {l.event.value}>
          </input>}
          <label className="lead">POR {i+1} Duration</label>
          <input  type="checkbox" 
            className = "mt-2 ml-2"
            style= {{width:`20px`, height:`20px`, lineHeight: `20px`}}
  ...
  const handleChange = (n, idx) => e => {
      let visualresumeCopy = visualresumepro;
      visualresumeCopy.porInformation.value[idx][n] = e.target.value;
      vr(visualresumeCopy);
    };
  
  const handleChangeQuill = (idx) => e => {
      let visualresumeCopy = visualresumepro;
      visualresumeCopy.porInformation.value[idx]["desc"] = e;
      vr(visualresumeCopy);
    };
  ...
  const handleChangeCheck = (n, idx) => e => {
      let visualresumeCopy = visualresumepro;
      if(e.target.checked){
        visualresumeCopy.porInformation.value[idx][n]["optional"] = true;
        vr(visualresumeCopy);
      }else{
        visualresumeCopy.porInformation.value[idx][n]["optional"] = false;
        vr(visualresumeCopy);
      }
    };
  ...
  const handleChangeOptional = (n, v, idx) => e => {
      let visualresumeCopy = visualresumepro;
      visualresumeCopy.porInformation.value[idx][n][v] = e.target.value;
      vr(visualresumeCopy);
    };
  
  const handleTitle = e => {
    let visualresumeCopy = visualresumepro;
    visualresumeCopy.porInformation.title = e.target.value;
    vr(visualresumeCopy);
  }
  
  return(
  <div className= "container">
      <label className="lead">Position of Responsibility</label>
      <input 
        type = "text"
        className="form-control"
        defaultValue={visualresumepro.porInformation.title}
    ...
    <Button className = "btn alert mr-4 mt-2" onClick = {handleAdd}>Add POR</Button>
  </div>
  )
}
