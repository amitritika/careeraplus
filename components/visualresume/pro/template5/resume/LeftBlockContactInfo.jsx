...
const LeftBlockContactInfo = (props) =>{
  ...
  return (
    <div id = {id} style = {{ height: `${textHeight}`, color: `white`, position: `absolute`, top: `${top}`, left: `${left}`, fontFamily: `calibri`, textAlign: `center`, fontWeight: `bold`}}>
      ...
      <div style = {{height: `${textHeight}`, width: `${textWidth}`, color: `${bg}`, lineHeight: `${textLine}`, position: `absolute`, left: `${textLeft}`, fontFamily: `calibri`, fontSize: `${textFont}`, textAlign: `left`, fontWeight: `regular`}}>{name}</div>
    </div>
  )
}
