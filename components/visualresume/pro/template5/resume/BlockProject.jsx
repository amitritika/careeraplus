...
const BlockProject = (props) =>{
  ...
  return (
    <div id = {id} style = {{height: `${height}`, width: `${width}`, position: `absolute`, top: `${top}`, left: `${left}`, color: `${bg}`, fontFamily: `calibri`, fontSize: `${size}`, textAlign: `left`}}>
      ...
      <p style = {{top: `${top1}`, position: `absolute`, color: `${bg}`, fontWeight: `bold`}}>{title}</p>
      {(desgShow || dateShow || clientShow) && <p style = {{top: `${top2}`, position: `absolute`}}>
        {desgShow && <span style = {{fontWeight: `bold`, color: `${borderColor}`}}>{desg} - </span>  }
        {dateShow && <span style = {{fontStyle: `italic`}}>{startD}-{endD} | </span>}
        {clientShow && <span style = {{fontStyle: `italic`, color: `${borderColor}`}}>{client}</span>}</p>}
