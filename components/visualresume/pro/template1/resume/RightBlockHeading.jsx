const RightBlockHeading = (props) =>{
  ...
  let width = (props.fac * 100).toString() + "px";
  ...
  let left = (props.fac * 30).toString() + "px";
  let top = (props.fac * props.props.top).toString() + "px";
  

  return (
    <div id = {id} style = {{height: `${height}`, width: `${width}`, color: `${bg}`, lineHeight: `${line}`, position: `absolute`, top: `${top}`, left: `${left}`, fontFamily: `calibri`, fontSize: `${size}`, textAlign: `center`, fontWeight: `bold`}}>
       {name}
    </div>
  )
}
