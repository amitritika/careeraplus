...
import Resume from "../../../components/visualresume/pro/template1/Resume"
import Page1 from "../../../components/visualresume/pro/template1/Page1"
import {visualresumedata} from "../../../helpers/visualresume/pro"
import {page, pagesRedistribution, componentSequence, colors} from "../../../helpers/visualresume/pro/template1/template1"
...
const Template1 = () => {
 ...
 const vr = (data, layoutInfoDisplay, userInfoDisplay) => {
   setValues({...values, visualresumepro: data, list: componentSequence(data, name, email, photo), 
							layoutInfoDisplay: layoutInfoDisplay, userInfoDisplay: userInfoDisplay});
 }
  const personalInformation = (data) =>{
    console.log(componentSequence(data.visualresumepro, data.name, data.email, data.photo))
    setValues({...values, name: data.name, username: data.username, email: data.email, photo: data.photo, visualresumepro:data.visualresumepro, 
               list: componentSequence(data.visualresumepro, data.name, data.email, data.photo)
               ,showLeftBlock: true})
  }
  ...
  const leftBlockComponent = (fac) =>{
    //let list = componentSequence(visualresumepro, name, email, photo);
    return (
              ...
              <Row>
                ...
                <Col xs = "2" lg = "1">
                  <div style = {{width: `50px`, height: `50px`, backgroundColor: `${font}`, marginTop: `10px`}}></div>
								...
								<Col xs= "12" lg = "4">
                  <UserInformationpro vr = {vr} pr = {personalInformation} type="expert" template = "template1" userInfoDisplay = {userInfoDisplay} ref = {editSection} />
                </Col>
